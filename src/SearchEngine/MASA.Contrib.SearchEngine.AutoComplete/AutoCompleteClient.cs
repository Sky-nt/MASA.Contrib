...
public class AutoCompleteClient : IAutoCompleteClient
{
    ...
    public AutoCompleteClient(IElasticClient elasticClient, string indexName)
    {
        _client = new DefaultMasaElasticClient(elasticClient);
    ...
    /// <typeparam name="TValue"></typeparam>
    /// <returns></returns>
    public async Task<SetResponse> SetByNotOverrideAsync<TDocument, TValue>(
        TDocument[] documents,
        CancellationToken cancellationToken = default) where TDocument : AutoCompleteDocument<TValue>
    {
